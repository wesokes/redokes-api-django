...
#        self.populate_missing_rows()
    ...
    def populate_missing_rows(self):
        ...
        while expected_index < num_expected:
            ...
            if (
                self.rows[existing_index]['year'] == current_date.year and
                self.rows[existing_index]['month'] == current_date.month and
                self.rows[existing_index]['day'] == current_date.day
            ):
            ...
            for row in self.rows:
                ...
                if found_record:
                    for aggregate_name in self.aggregates.keys():
                        new_row['{0}__{1}'.format(
                            self.legend_map_prefix,
                            row[self.legend_map_field]
                        )] = row[aggregate_name]
